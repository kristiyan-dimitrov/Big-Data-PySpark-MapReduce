"""
I will train a Gradient Boosted Trees Classfier on the data
...
Note: There is an almost identical .py file where I do the same but with a Random Forest
...
gbt = GBTClassifier(featuresCol = "features", labelCol = "ArrestIdx")

# Pipeline
pipeline = Pipeline(stages = [BeatIdx, DistrictIdx, DomesticIdx, Primary_TypeIdx, Community_AreaIdx, WardIdx, Hour_of_dayIdx, IUCRIdx, Day_of_weekIdx, MonthIdx, ArrestIdx, encoder, vectorAssembler, gbt])
...
# Writing output
with open("dimitrov_exercise4_output.txt", 'w') as output:
    output.write("The Area under the ROC of the GBTClassifier is: " + str(AUC) + '\n')
    output.write("The Accuracy of the GBTClassifier is: " + accuracy + '\n')
